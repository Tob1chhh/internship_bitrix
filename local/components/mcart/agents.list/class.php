...
class AgentsList extends CBitrixComponent implements Controllerable, Errorable
{
    ...
    final public function onPrepareComponentParams($arParams): array
    {
        ...
        if (isset($arParams["AGENTS_COUNT"])) {
            $arParams["AGENTS_COUNT"] = "3";
        } else {
            $arParams["AGENTS_COUNT"] = strval($arParams["AGENTS_COUNT"]);
        }
    ...
    private function getAgents(string $entity, array $arTypeAgents): array
    {
        ...
        $nav->allowAllRecords(true)->setPageSize($this->arParams['AGENTS_COUNT']) //Нужно передать параметр Количество элементов из мввсива $this->arParams
            ->initFromUri();
        ...
        while ($arAgent = $rsAgents->fetch()) {
            foreach ($arTypeAgents as $arTypes) {
                if ($arTypes["ID"] == $arAgent['UF_TYPE']) {
                    $arAgent['UF_TYPE'] = $arTypes["VALUE"];
                }
            }
            if (isset($arAgent["UF_PHOTO"])) {
    ...
    public function clickStarAction($agentID)
    {
        ...
        $props = CUserOptions::GetOption("mcart_agent", "options_agents_star");
        if (isset($props)) {
            if(!is_array($props)) {
                $props = (array)$props;
            }
            if (!in_array($agentID, $props)) {
                $props[] = $agentID;
            } else {
                unset($props[array_search($agentID, $props)]);
            }
            foreach($props as $el) {
                $value[] = $el;
            } 
